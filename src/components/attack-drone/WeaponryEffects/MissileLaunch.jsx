...
const MissileLaunch = ({ startPosition, targetPosition, onComplete }) => {
  ...
  useFrame(() => {
    ...
    if (missileRef.current && !exploding) {
      ...
      // Play explosion sound 4 seconds BEFORE reaching the target
      if (!soundPlayed && progress > 0.5 && (flightDuration - elapsed) <= explosionDelay) {
        // Play explosion sound - early by 4 seconds
        try {
      ...
      // When missile reaches target, trigger explosion
      if (progress === 1) {
        setExploding(true);
